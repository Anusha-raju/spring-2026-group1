...
\begin{figure}[H]
\centering
\includegraphics[width=0.72\textwidth]{figs/current_architecture.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.62\textwidth]{figs/ragflow.pdf}
...
The pipeline operates in a one-shot manner. For each input query, the retriever gathers relevant chunks using the query text, the agent’s professional profile, and other scenario-specific context. These retrieved chunks are then combined with the original query and passed into the agent’s prompt.

Once the prompt is prepared, GPT-5.2 is invoked a single time to generate the final output. An earlier version of the system used a two-step process, where prompt synthesis and response generation were handled by separate API calls. However, this additional complexity was later removed after being found unnecessary.
